...
const Mansonry = () => {
    ...
    const eyeIcon = `<svg width="20px" height="20px" viewBox="0 0 24 24"> <path d="M15.0007 12C15.0007 13.6569 13.6576 15 12.0007 15C10.3439 15 9.00073 13.6569 9.00073 12C9.00073 10.3431 10.3439 9 12.0007 9C13.6576 9 15.0007 10.3431 15.0007 12Z" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/> <path d="M12.0012 5C7.52354 5 3.73326 7.94288 2.45898 12C3.73324 16.0571 7.52354 19 12.0012 19C16.4788 19 20.2691 16.0571 21.5434 12C20.2691 7.94291 16.4788 5 12.0012 5Z" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/> </svg>`
                ...
                <>
                    <div className="fixed right-0 top-[5%] h-dvh w-full z-50">
                        ...
                        <div className='dark:bg-[#3a3b3c] bg-[#dddddd] px-8 py-3 h-screen overflow-y-auto'>
                            <button onClick={() => setViewFaves(false)} className='w-5 h-5 rounded-full p-2 leading-[0] z-50 cursor-pointer float-right -translate-x-[18px]'>X</button>
                            ...
                            <div className="flex flex-wrap gap-5 justify-stretch mx-auto py-8 fav">
                                {Object.entries(favorite).map((item, index) => (
                                    ...
                                    {item[1] ? 
                                        <figure className="relative [&:has(:hover)_img]:brightness-75 duration-100 [&:has(svg:hover)_img]:brightness-50 h-full">
                    ...
                    <span key={pastinha.cat}>
                        {pastinha.img.map(({ url, timeCreated }, index) => (
                            <figure key={index} className={`item ${pastinha.cat.toLowerCase()} [&:has(img:hover)_button]:opacity-100 [&:has(button:hover)_button]:opacity-100 grid place-items-center`} >
                               ...
                               <Image
                                    className="bg z-40 w-full h-full blur-[50px] animate-[scaling_.5s_forwards]"
                                    style={{animationDelay:`${Math.random() * index / 15}s`}}
                                    src={url}
                ...
                {modal && (
                    <div className="fixed z-[60] w-full h-dvh backdrop-blur-md backdrop-saturate-[.1] backdrop-brightness-[.3] saturate-[1.3] left-0 top-0 grid place-items-center">
                        <a onClick={() =>setModal(false)} 
                            className={`bg-black w-8 h-8 rounded-full p-2 indent-[3px] leading-none cursor-pointer absolute z-50 text-gray-300 hover:text-white md:top-10 md:right-12 top-[unset] right-[unset] left-1/2 bottom-8 md:left-[unset] md:bottom-[unset]`}>X</a>
